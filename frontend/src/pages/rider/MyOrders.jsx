...
/* ─────────────────────────────────────────────────────── */
/*  Earnings Breakdown Panel (Assigned + New Tasks cards)  */
/* ─────────────────────────────────────────────────────── */
      ...
      {hasDistance && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-violet-50 border-b border-violet-100">
          ...
          <span className="text-[11px] font-bold text-violet-700">
            ...
            <span className="text-violet-900 font-black">{distance_km} KM</span>
          </span>
        </div>
      )}
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-semibold text-slate-600">Base Pay</span>
          </div>
          <span className="text-[12px] font-black text-slate-800">₹{base_pay.toFixed(0)}</span>
        </div>
            ...
            <span className="text-[11px] font-semibold text-slate-600">
              ...
              {hasDistance && (
                <span className="ml-1 text-slate-400 font-normal">
                  ({distance_km} km × ₹{petrol_rate})
                </span>
              )}
            </span>
          </div>
          <span className="text-[12px] font-black text-emerald-700">₹{distance_allowance.toFixed(0)}</span>
        </div>

        {/* Bonus Incentive */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className={clsx("w-2 h-2 rounded-full", bonus_incentive > 0 ? "bg-amber-400" : "bg-slate-200")} />
            <span className="text-[11px] font-semibold text-slate-600 flex items-center gap-1">
              Bonus Incentive
              {bonus_incentive > 0 && <Zap size={10} className="text-amber-500" />}
            </span>
          </div>
          <span className={clsx("text-[12px] font-black", bonus_incentive > 0 ? "text-amber-600" : "text-slate-400")}>
            {bonus_incentive > 0 ? `₹${bonus_incentive.toFixed(0)}` : "₹0"}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className={clsx("w-2 h-2 rounded-full", penalty_risk > 0 ? "bg-red-400" : "bg-slate-200")} />
            <span className="text-[11px] font-semibold text-slate-600 flex items-center gap-1">
              Penalty Risk
              {penalty_risk > 0 && <AlertTriangle size={10} className="text-red-500" />}
            </span>
          </div>
          <span className={clsx("text-[12px] font-black", penalty_risk > 0 ? "text-red-600" : "text-slate-400")}>
            {penalty_risk > 0 ? `-₹${penalty_risk.toFixed(0)}` : "₹0"}
          </span>
        </div>
...
/* ─────────────────────────────────────────────────────── */
/*  Final Earnings Badge (Completed cards)                 */
/* ─────────────────────────────────────────────────────── */
...
const MyOrders = () => {
  ...
  /* ── Real-Time GPS Tracking for in-transit orders ── */
  useEffect(() => {
    let interval;
  ...
  /* ── helper: normalise earning value to a display number ── */
  const getEarningTotal = (earning) => {
    if (!earning) return 0;
    if (typeof earning === "object") return earning.total || 0;
    return parseFloat(earning) || 0;
  };
        ...
        <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm self-end md:self-auto">
          <button
            ...
            className={`p-2.5 rounded-xl transition-all ${
              viewMode === "grid" ? "bg-brand-purple text-white shadow-md" : "text-slate-400 hover:bg-slate-50"
            }`}
          >
            <LayoutGrid size={20} />
          </button>
          <button
            ...
            className={`p-2.5 rounded-xl transition-all ${
              viewMode === "list" ? "bg-brand-purple text-white shadow-md" : "text-slate-400 hover:bg-slate-50"
            }`}
          >
            <List size={20} />
          </button>
            ...
            filteredOrders.map((order) => (
              ...
              >
                ...
                <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                  ...
                  <div className="flex items-center gap-3">
                    {/* Earning badge — show total for non-assigned, full badge for completed */}
                    {activeTab === "Delivered" ? (
                      ...
                      <div className="text-right">
                        <div className="text-xs font-black text-slate-400 uppercase tracking-widest leading-none mb-1">
                          {activeTab === "Assigned" || activeTab === "New" ? "Est. Earn" : "Earning"}
                        </div>
                ...
                {/* ── Earnings Breakdown (Assigned + New Tasks only) ── */}
                {(activeTab === "Assigned" || activeTab === "New") &&
                  typeof order.estimated_earning === "object" && (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 group-hover:border-brand-blue/30 transition-colors">
                      <h5 className="font-black text-slate-900 mb-1">
                        {order.vendor_info?.shop_name || "Vendor Shop"}
                      </h5>
