    ...
    if (loading && !stats.active_riders) {
        return (
            <div className="h-[80vh] flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-slate-500 font-normal animate-pulse">Syncing Live Fleet Data...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-medium text-slate-800 tracking-tight">Global Tracking Dashboard</h1>
                    <p className="text-slate-500 font-medium">Real-time overview of all riders and active shipments.</p>
                </div>
                <div className="text-right">
                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-1">Last Updated</p>
                    <p className="text-xs font-normal text-emerald-500">{new Date().toLocaleTimeString()}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 flex items-center justify-between shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all group">
                    <div>
                        <p className="text-xs font-medium text-slate-400 uppercase tracking-[0.2em]">Active Riders</p>
                        <h2 className="text-5xl font-medium text-slate-900 mt-2 tracking-tighter">{stats.active_riders}</h2>
                    </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 flex items-center justify-between shadow-sm hover:shadow-xl hover:shadow-amber-500/5 transition-all group">
                    <div>
                        <p className="text-xs font-medium text-slate-400 uppercase tracking-[0.2em]">In Transit</p>
                        <h2 className="text-5xl font-medium text-slate-900 mt-2 tracking-tighter">{stats.in_transit_orders}</h2>
                    </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-rose-100 flex items-center justify-between shadow-sm shadow-rose-100 hover:shadow-xl hover:shadow-rose-500/5 transition-all group">
                    <div>
                        <p className="text-xs font-medium text-rose-400 uppercase tracking-[0.2em]">Failed/Delayed</p>
                        <h2 className="text-5xl font-medium text-rose-600 mt-2 tracking-tighter">{stats.failed_delayed_orders}</h2>
                    </div>
