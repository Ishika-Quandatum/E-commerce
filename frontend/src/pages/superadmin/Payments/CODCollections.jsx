    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-24">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl font-medium text-slate-900 tracking-tight font-title italic">COD <span className="text-brand-blue not-italic">Collections</span></h1>
            <p className="text-slate-500 font-normal mt-1">Monitor and record cash submissions from delivery riders.</p>
          </div>
        </div>
        
        <div className="flex items-center gap-4 bg-white p-3 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40">
           <button className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl font-normal text-sm hover:bg-slate-800 transition-all shadow-lg shadow-slate-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-sm space-y-6 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                  <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-400 mb-1">Total COD Collected</p>
                  <h4 className="text-4xl font-medium tracking-tighter text-slate-900">₹{(totals.pending + totals.submitted).toLocaleString()}</h4>
              </div>
              <div className="flex items-center gap-2 text-slate-400 text-[10px] font-medium uppercase tracking-widest relative z-10">
          ...
          <div className="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-sm space-y-6 group">
              ...
              <div>
                  <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-400 mb-1">Pending Submission</p>
                  <h4 className="text-4xl font-medium tracking-tighter text-slate-900">₹{totals.pending.toLocaleString()}</h4>
              </div>
              <div className={clsx(
          ...
          <div className="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-sm space-y-6 group">
              ...
              <div>
                  <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-400 mb-1">Total Submitted</p>
                  <h4 className="text-4xl font-medium tracking-tighter text-slate-900">₹{totals.submitted.toLocaleString()}</h4>
              </div>
        ...
        <div className="p-10 border-b border-slate-50 bg-slate-50/30 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4 bg-white px-6 py-4 rounded-2xl border border-slate-100 shadow-sm w-full md:w-96 group focus-within:ring-2 focus-within:ring-brand-blue/20 transition-all">
            ...
            <input 
              ...
              className="bg-transparent border-none focus:ring-0 text-sm font-normal w-full text-slate-900 placeholder:text-slate-300"
              value={searchTerm}
                ...
                <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-10 py-8">
                    <div className="flex flex-col gap-1">
                      <span className="text-sm font-medium text-slate-900 flex items-center gap-2 group-hover:text-brand-blue transition-colors">
                  ...
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-3">
                       <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-medium text-xs">
                          {c.rider_name?.split(' ').map(n => n[0]).join('') || 'R'}
                       </div>
                       <div>
                          <div className="text-sm font-medium text-slate-900">{c.rider_name}</div>
                          <div className="text-[10px] font-normal text-slate-400 uppercase tracking-tighter">ID: #{c.rider}</div>
                       </div>
                    </div>
                  </td>
                  ...
                  <td className="px-10 py-8">
                    <div className="flex justify-center">
                      <span className={clsx(
                        "px-4 py-2 rounded-xl text-[10px] font-medium uppercase tracking-widest",
                  ...
                  <td className="px-10 py-8 text-right">
                    {c.status === 'Pending' ? (
                        <button 
                            ...
                            className="bg-brand-blue text-white px-6 py-3 rounded-2xl text-xs font-medium uppercase tracking-widest hover:bg-slate-900 transition-all shadow-lg shadow-brand-blue/20 active:scale-95"
                        >
                            Record Submission
                        </button>
                    ) : (
                        <div className="flex flex-col items-end gap-1">
                            <div className={clsx(
                                "flex items-center gap-2 font-medium text-xs uppercase tracking-widest",
                            ...
                            {c.status === 'Disputed' && (
                                <span className="text-[10px] font-normal text-rose-400 italic">
